float getFireflyNoise(vec3 pos, float height){
	...
	if (ymult < 2.0){
		noise+= getVolumetricNoise0(pos + vec3(frametime * 0.2, frametime * 0.05, frametime * 0.1)) * 10.1;
	}
...
vec3 GetFireflies(float pixeldepth0, vec3 color, float dither) {
	...
	if (visibility > 0.0) {
		...
		for(int i = 0; i < 64; i++) {
			...
			if (length(worldposition.xz) < 256.0 && depth0 > minDist){
				vec3 col = vec3(1.0);

				if (depth0 < minDist) col *= color;
				
				float noise = getFireflyNoise(worldposition.xyz + cameraPosition.xyz, 100.0);
