...
vec3 GetFogColor(vec3 viewPos, bool layer) {
	...
	vec3 fog = vec3(0.0);
    #if FOG_COLOR_MODE == 1
    fog = fogCol * baseGradient * vec3(FOG_R, FOG_G, FOG_B) * FOG_I;
	...
	if (layer) lightFog = GetSkyColor(viewPos, false) * baseGradient * FOG_I;
	if (!layer){
        #if FOG_COLOR_MODE == 1
...
/*
This should be copatible with mesa drivers, but it doesn't work as intended. Don't even look at this code, it's useless
#if (WATER_MODE == 1 || WATER_MODE == 3) && !defined SKY_VANILLA && !defined NETHER
uniform vec3 fogColor;
#endif
...
#ifdef OVERWORLD
vec3 GetFogColor(vec3 viewPos, vec3 worldPos, float layer, float density) {
	vec3 nViewPos = normalize(viewPos);
	float lViewPos = length(viewPos) / 64.0;
	lViewPos = 1.0 - exp(-lViewPos * lViewPos);
	vec2 pos = (cameraPosition.xz + worldPos.xz);
	...
	if (layer == 0.0) density *= FIRST_LAYER_DENSITY;
	if (layer == 1.0) density *= SECOND_LAYER_DENSITY;
	if (isEyeInWater == 1) density *= 0.0;
    float nightDensity = NIGHT_FOG_DENSITY;
    float weatherDensity = WEATHER_FOG_DENSITY;
    float exposure = exp2(timeBrightness * 0.75 - 1.00);
	...
	vec3 lightFog = vec3(0.0);
	if (layer == 1.0) lightFog = GetSkyColor(viewPos, false) * baseGradient * FOG_I;
	if (layer == 0.0){
        #if FOG_COLOR_MODE == 1
        lightFog = pow(fogcolorSun, vec3(4.0 - sunVisibility)) * baseGradient * FOG_I;
        #elif FOG_COLOR_MODE == 0
...
void GetFirstFogLayer(in vec3 viewPos, in vec3 worldPos, in float density, inout vec3 color){
	density *= FIRST_LAYER_DENSITY;
	...
	vec3 fogColor = GetFogColor(viewPos, worldPos, 0.0, density);
...
void GetSecondFogLayer(in vec3 viewPos, in vec3 worldPos, in float density, inout vec3 color){
	density *= SECOND_LAYER_DENSITY;
	...
	vec3 fogColor = GetFogColor(viewPos, worldPos, 1.0, density);
...
void GetDistantFade(in vec3 viewPos, in vec3 worldPos, in float density, inout vec3 color){
	float dither = Bayer64(gl_FragCoord.xy);
	...
	vec3 fogColor = GetSkyColor(viewPos.xyz, false);
...
void Fog(inout vec3 color, vec3 viewPos) {
	...
	GetDistantFade(viewPos, worldPos.xyz, density, color);

	#ifdef OVERWORLD
	GetFirstFogLayer(viewPos, worldPos.xyz, density, color);
	GetSecondFogLayer(viewPos, worldPos.xyz, density, color);
	#endif
